...
from ot_backprop_pnwo.evaluation.evaluation_util import ConvergenceConfig, exec_log_model_evaluation_two_phase
...
if __name__ == '__main__':
    ...
    l_log_pn_pairs = (
        ...
        ('Sepsis', 'logs/sepsis/sepsis.xes', 'models/sepsis/cold-start/sepsis_DFM_02.pnml'),
    )
    ot_sizes = ((500, 500), )
    convergence_configs = (ConvergenceConfig(nbr_iterations_min=50, nbr_iterations_max=5000, eps_convergence=0.0025),
                           ConvergenceConfig(nbr_iterations_min=3000, nbr_iterations_max=5000, eps_convergence=0.0025))
    iterations = 12
    ...
    for (log_name, log_path, pn_path) in l_log_pn_pairs:
        ...
        for path_pn in pn_paths:
            ...
            exec_log_model_evaluation_two_phase(log_evaluation_reporter=log_evaluation_reporter, 
                                                df_ev=df_ev, net=net, im=im, fm=fm, name_log=log_name, name_spn=name_spn, 
                                                convergence_configs=convergence_configs,
                                                ot_sizes=ot_sizes, hot_start=False, iterations=iterations, only_phase_one=True)
